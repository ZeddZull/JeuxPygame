...
class JeuSnake(object):
	...
	def demarrer(self):
					...
					for event in pygame.event.get():
						if event.type == KEYDOWN:
							if event.key == K_LEFT:
								...
								pause = True
								self.afficherMessage("Jeu en pause")
								while pause:
									ev = pygame.event.wait()
									if ev.type == KEYDOWN:
										if ev.key == K_SPACE:
											pause = False
											self.afficherMessage("Fin de pause")
		...
		if not gg:
			...
			self.afficherMessage("Bien jouer vous avez terminer le jeu pour le moment é!")
		c = Classement("scoreSnake")
		c.ajout(Score("Zedd",len(self.serpent.corps)))
		c.sauver()
		time.sleep(2)
